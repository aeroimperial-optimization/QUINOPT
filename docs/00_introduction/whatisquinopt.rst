...
In the problem above, :math:`Q_{\gamma}(x,u(x),u'(x),...,u^k(x))` is a homogeneous quadratic polynomial in :math:`u(x),u'(x),...,u^k(x)`, it is polynomial in in :math:`x`, and it is affine in :math:`\gamma`. Moreover, :math:`\mathcal{H}` is the subspace of functions that satisfy :math:`m` homogeneous boundary conditions,  i.e.
...
.. note::
   QUINOPT can also handle problems with more dependent variables, i.e. :math:`u:[a,b]\to\mathbb{R}^q`, and problems in which the boundary values of the deendent variables and their derivatives appear explicitly in the integrand of the inequality constraint.
